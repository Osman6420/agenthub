...
- Merge `P2.6.0` first, then this P2.6.10 branch in the first parallel wave. If another branch has
  already allocated ingestion migration `0011` or ADR `0011`, the integration owner must renumber
  this branch before merge, then rerun migration drift, full PostgreSQL and full SQLite suites.
