...
**Proposed; planning only.** No code, schema, deployment or authorization behavior changes here.
...
1. Contract/ADR: lifecycle, timeouts, outbox, retry classes, readiness exposure.
2. Data/dispatch: additive models, constraints, outbox, transactional creation/claim.
3. Worker/reconciliation: heartbeat, progress, recovery, bounded periodic task.
4. Operations: Compose/host contract, preflight, worker probes, metrics/alerts.
5. Console: tenant-safe status/polling/recovery after response contract freezes.
6. Integration: real-service smoke and restart/failure drills.
