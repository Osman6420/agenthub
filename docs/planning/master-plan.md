...
Parts 1 and 2 are implemented and verified: exact-run containment is closed, scenario callability is
explicit, and served-index promotion/rollback now changes one coherent retrieval pointer. Part 3
exact-role release and runtime controls is the next milestone.
