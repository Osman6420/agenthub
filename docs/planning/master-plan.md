...
Part 1 exact run authorization and PostgreSQL containment is implemented and verified; Part 2 is
the next release-blocking milestone.
...
| UI, authorization and lifecycle closure (Phase 2.9) | **In progress; Part 1 implemented and verified 2026-08-01.** Exact-scenario run disclosure is closed across list/detail/control, persisted wait authority is current, and the non-owner PostgreSQL path matches canonical provisioning. Part 2 remains the next release-blocking P0 lifecycle correction; Parts 3–7 follow. | Phase 2.8 exact responsibilities and unified scenario/release/run/document lifecycles; canonical PostgreSQL/pgvector/RLS profile | [Phase 2.9 component plan](components/phase-2-9-ui-authorization-lifecycle-closure-plan.md) | No Part 1 ADR required; existing exact-responsibility contract was enforced, not changed | [Part 1 verification](archive/phase-2-9-part-1-exact-run-authorization-postgresql-containment-2026-08-01/verification.md), [audit report](../tasks/current-application-role-ui-audit/report.md) |
