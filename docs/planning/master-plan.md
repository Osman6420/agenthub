...
P6 (authored, governed agent system prompt), and P7.1 (the deny-by-default `DocumentParser` interface
+ dependency-free stdlib parsers — text/markdown/csv/json/html — wired into the staged-build
text-extraction seam) are implemented and verified. Phase 2 implementation is in progress (next: P7.2
— opt-in pdf/docx/xlsx parser adapters, **blocked on the document-parser dependency approval**; then
P7.3 external OCR egress and P7.4 Confluence/generic-REST connectors, **blocked on their egress
sign-off**); later live-egress/dependency milestones retain their explicit gates.
