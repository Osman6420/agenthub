...
def test_inventory_classifies_direct_tenant_tables() -> None:
    ...
    assert inventory["catalog.scenario"].classification == TenantTableClass.INDIRECT
    assert inventory["catalog.scenario"].tenant_column == ""
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.skipif(connection.vendor != "postgresql", reason="RLS requires PostgreSQL")
def test_readiness_rejects_missing_role_and_unprotected_table() -> None:
    ...
    assert "documents_document:RLS_DISABLED" in report.issues
    assert "documents_document:RLS_NOT_FORCED" in report.issues
    assert "documents_document:POLICY_MISSING_OR_NONCANONICAL" in report.issues
